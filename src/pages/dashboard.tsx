...
import { getAttachments } from 'apiSdk/attachments';
import { AttachmentInterface } from 'interfaces/attachment';

import { ChannelListPage as ShowCaseList } from 'pages/channels';
...
function DashboardPage() {
  ...
  const channelStat = useSWR<PaginatedInterface<ChannelInterface>>(
    () => `/channels`,
    () => getChannels({ limit: 1, offset: 0 }),
  );

  const attachmentStat = useSWR<PaginatedInterface<AttachmentInterface>>(
    () => `/attachments`,
    () => getAttachments({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Channels',
      result: channelStat?.data?.totalCount,
      isLoading: channelStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Attachments',
      result: attachmentStat?.data?.totalCount,
      isLoading: attachmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
